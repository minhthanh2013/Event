...
export class PopularityService {
    ...
    async checkUnique(popularity: PopularityDto): Promise<ResponseData> {
        const userInfo = (await this.userRepository.findOne({where: {user_id: popularity.userId}}))
        const conf = (await this.conferenceRepository.findOne({where: {conference_id: popularity.conferenceId}}))
        const conf_cate_id = conf.conference_category
        const cate = (await this.confCateRepository.findOne({where: {category_id: conf_cate_id}})).category_name;
        ...
        const data = await this.popularRepostory.findOne({where: {viewer_id: popularity.userId, conference_id: popularity.conferenceId}})
        if (data !== null) {
            console.log(2)
            response.data = (popular / totalUserSameCategory * 100).toFixed(2)
            response.status = true
        ...
        if (check) {
            await this.popularRepostory.insert({viewer_id: popularity.userId, conference_id: popularity.conferenceId})
            await this.conferenceRepository.increment({conference_id: popularity.conferenceId}, 'popularity', 1)
            response.data = (popular / totalUserSameCategory * 100).toFixed(2)
            response.status = true
        }
        return response
